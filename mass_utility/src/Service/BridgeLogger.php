...
/**
 * Bridge-specific flat-file logging service.
 */
class BridgeLogger
{
    ...
    {
        $this->logPath = _PS_MODULE_DIR_ . 'mass_utility/logs/mass_utility.log';
        $logDir = dirname($this->logPath);
        if (!is_dir($logDir)) {
            try {
                $success = mkdir($logDir, 0755, true);
                if (!$success && !is_dir($logDir)) {
                    error_log('MassUtility BridgeLogger: Failed to create logs directory at ' . $logDir);
                }
            } catch (\Throwable $e) {
                if (!is_dir($logDir)) {
                    error_log('MassUtility BridgeLogger: Exception creating logs directory - ' . $e->getMessage());
                }
            }
        }
    }
    ...
    public function log(string $message, string $level = 'INFO'): void
    {
        ...
        try {
            ...
            if ($result === false) {
                error_log('MassUtility BridgeLogger: Failed to write to log file: ' . $this->logPath);
            }
        } catch (\Throwable $e) {
            error_log('MassUtility BridgeLogger: Exception writing to log file - ' . $e->getMessage());
        }
    }
